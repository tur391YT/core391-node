  ...
  if (visualEditor) {
    ...
    visualEditor.addEventListener('click', (e) => {
      if (e.target.tagName === 'IMG') {
        ...
        const defaultVal =
          currentSrc.includes('placeholder') || currentSrc === '' ? '' : currentSrc;

        const newSrc = prompt(
          'Вставьте относительный путь (например: img/weapon.png) или URL к изображению:',
          defaultVal,
        );
    ...
    visualEditor.addEventListener('paste', (e) => {
      ...
      reader.onload = (event) => {
        ...
        img.onload = () => {
          ...
          const iconWrapper = sel.anchorNode?.parentElement?.closest(
            '.wp-item-blank-icon, .wp-slot, .wp-cell-center',
          );
...
<<<<<<< HEAD
  ...
  window.addSlot = function () {
    ...
    if (!container) {
      alert(
        'Сначала добавьте блок "+ Отряд", затем поставьте курсор внутрь него и нажмите "+ Слот отряда"',
      );
      return;
    }
